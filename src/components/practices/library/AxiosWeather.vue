...
// 교재 224p는 API_KEY를 소스에 직접 적지만, 이 저장소는 Public으로 제출하므로
// 키를 .env.local(gitignore 대상)로 분리하고 환경변수로 주입받는다.
const API_KEY = import.meta.env.VITE_OPENWEATHER_API_KEY

const handleFetchWeather = async () => {
  if (!API_KEY) {
    alert('VITE_OPENWEATHER_API_KEY가 없습니다. .env.example을 참고해 .env.local을 만드세요.')
    return
  }
  ...
  try {
    ...
    // Axios에서는 응답(response.data)이 자동으로 JSON 파싱된다.
